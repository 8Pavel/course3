...
from utils import PostManager, CommentManager
...
def search():
    ...
    return render_template('search.html', post=posts)


@html_bp.route('/users/<string:username>', methods=['GET'])
def user_feed(username: str):
    posts = post_manager.get_post_by_pk(username)

    return render_template('user-feed', posts=posts)
